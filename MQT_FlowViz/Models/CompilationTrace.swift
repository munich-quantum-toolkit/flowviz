...
final class CompilationTrace: Codable, Identifiable {
  ...
  init(
    id: String, circuitName: String, figureOfMerit: String, mdpPolicy: String,
    device: DeviceMetadata, schemaVersion: String, timestamp: Double, totalDuration: Float,
  ...
  @MainActor func getActionEvolution() -> [(
    title: String, ranges: [ClosedRange<Int>], type: ActionType
  )] {
    ...
    for step in sortedSteps {
      ...
      processStepForRange(
        isActive: true, stepIndex: step.stepIndex, ranges: &ranges, start: &start, prev: &prev)
    ...
    for action in orderedActions {
      ...
      processStepForRange(
        isActive: false, stepIndex: 0, ranges: &ranges, start: &start, prev: &prev)
      actionRanges[action] = ranges
    }
  ...
  func getMDPStateEvolution() -> (
    synthesizedRanges: [ClosedRange<Int>], laidOutRanges: [ClosedRange<Int>],
    routedRanges: [ClosedRange<Int>]
  ) {
    guard !steps.isEmpty else { return ([], [], []) }
    ...
    var synStart: Int? = nil
    var synPrev: Int? = nil
    var laidStart: Int? = nil
    var laidPrev: Int? = nil
    var routStart: Int? = nil
    var routPrev: Int? = nil

    for step in sortedSteps {
      let idx = step.stepIndex
      processStepForRange(
        isActive: step.synthesized, stepIndex: idx, ranges: &synRanges, start: &synStart,
        prev: &synPrev)
      processStepForRange(
        isActive: step.laidOut, stepIndex: idx, ranges: &laidRanges, start: &laidStart,
        prev: &laidPrev)
      processStepForRange(
        isActive: step.routed, stepIndex: idx, ranges: &routRanges, start: &routStart,
        prev: &routPrev)
    }
    ...
    // Setting isActive to false causes the helper to close any active range
    processStepForRange(
      isActive: false, stepIndex: 0, ranges: &synRanges, start: &synStart, prev: &synPrev)
    processStepForRange(
      isActive: false, stepIndex: 0, ranges: &laidRanges, start: &laidStart, prev: &laidPrev)
    processStepForRange(
      isActive: false, stepIndex: 0, ranges: &routRanges, start: &routStart, prev: &routPrev)

    return (synRanges, laidRanges, routRanges)
  }

  // helper to cleanly process the ranges
  private func processStepForRange(
    isActive: Bool, stepIndex: Int, ranges: inout [ClosedRange<Int>], start: inout Int?,
    prev: inout Int?
  ) {
