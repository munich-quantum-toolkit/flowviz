  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 24) {
      ...
      VStack(spacing: 16) {
        HStack(spacing: 16) {
          GraphBoxView(
            title: "Parallelism", chartData: parallelismData, chartColor: Color.bluePrimary
          )
          .aspectRatio(1, contentMode: .fit)

          GraphBoxView(
            title: "Entanglement Ratio", chartData: entanglementData, chartColor: Color.bluePrimary
          )
          .aspectRatio(1, contentMode: .fit)
    ...
    .task(id: trace.id) {
      parallelismData = trace.steps.map {
        ChartDataPoint(step: $0.stepIndex, value: Float($0.parallelism))
      }
      entanglementData = trace.steps.map {
        ChartDataPoint(step: $0.stepIndex, value: Float($0.entanglementRatio))
      }
      livenessData = trace.steps.map {
        ChartDataPoint(step: $0.stepIndex, value: Float($0.liveness))
      }
      qubitsData = trace.steps.map {
        ChartDataPoint(step: $0.stepIndex, value: Float($0.numQubits))
      }
      rewardData = trace.steps.map { ChartDataPoint(step: $0.stepIndex, value: Float($0.reward)) }
      depthData = trace.steps.map {
        ChartDataPoint(step: $0.stepIndex, value: Float($0.rawCriticalDepth))
      }
      ...
      for step in trace.steps {
        ...
        tempExpectedFidelity.append(
          ChartDataPoint(step: index, value: Float(fidelity.value), kind: fidelity.kind))

        let criticalDepth = step.figuresOfMerit.criticalDepth
        tempCriticalDepth.append(
          ChartDataPoint(step: index, value: Float(criticalDepth.value), kind: criticalDepth.kind))

        if let hellingerDistance = step.figuresOfMerit.estimatedHellingerDistance {
          tempHellingerDistance.append(
            ChartDataPoint(
              step: index, value: Float(hellingerDistance.value), kind: hellingerDistance.kind))
        } else {
          tempHellingerDistance.append(nil)
        }

        if let esp = step.figuresOfMerit.estimatedSuccessProbability {
          tempEstimatedSuccessProbability.append(
            ChartDataPoint(step: index, value: Float(esp.value), kind: esp.kind))
        } else {
      ...
      var tempFOMSeries = [
        ChartDataSet(
          name: "Expected Fidelity", color: Color.bluePrimary, data: tempExpectedFidelity),
      ...
      if filteredHellinger.count == count {
        tempFOMSeries.append(
          ChartDataSet(
            name: "Hellinger Distance", color: Color.yellowPrimary,
            data: tempHellingerDistance.compactMap(\.self)))
      }

      if filteredESP.count == count {
        tempFOMSeries.append(
          ChartDataSet(
            name: "Estimated Success Probability", color: Color.greenPrimary,
            data: tempEstimatedSuccessProbability.compactMap(\.self)))
      }
