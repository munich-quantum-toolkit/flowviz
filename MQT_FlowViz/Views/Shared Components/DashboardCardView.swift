...
struct DashboardCardView<Content: View>: View {
  ...
  init(
    title: String? = nil, dotmarkColor: Color? = nil, dotmarkHelpText: String? = nil,
          ...
          if let markColor = dotmarkColor, let helpText = dotmarkHelpText {
            Circle()
              ...
              .help(helpText)
                ...
                .popover(
                  isPresented: $showActionTooltip, attachmentAnchor: .point(.bottom),
                  arrowEdge: .bottom
                ) {
                  Text(helpText)
                  ...
                  .padding(.vertical, 8)
                  // Forces it to stay a small popover on iPhone/iPad.
