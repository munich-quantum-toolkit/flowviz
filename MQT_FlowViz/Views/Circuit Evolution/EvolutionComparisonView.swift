...
struct EvolutionComparisonView: View {
  ...
  init(trace: CompilationTrace, currentStep: Int) {
    ...
    if currentStep < trace.steps.count - 1 {
      self._comparedStep =
        currentStep == 0
        ? State(initialValue: trace.steps.count - 1) : State(initialValue: currentStep + 1)
    } else {
  ...
  var body: some View {
    let dynamicLayout =
      layoutHorizontal
      ? AnyLayout(HStackLayout(alignment: .top, spacing: 24)) : AnyLayout(VStackLayout(spacing: 24))

    ScrollView {
      dynamicLayout {
        circuitPanel(
          step: $currentStep, circuit: currentCircuit, parseError: parseErrorCurrentCircuit)
        circuitPanel(
          step: $comparedStep, circuit: comparedCircuit, parseError: parseErrorComparedCircuit)
      }
      .padding()
    }
  ...
  @ViewBuilder
  private func circuitPanel(step: Binding<Int>, circuit: ParsedCircuit?, parseError: String?)
    -> some View
  {
    let currentStep =
      (step.wrappedValue >= 0 && step.wrappedValue < trace.steps.count)
      ? trace.steps[step.wrappedValue] : nil
    ...
    VStack(alignment: .leading, spacing: 12) {
        ...
        if let currentStep = currentStep, let parsed = circuit {
          ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 18) {
              ...
              Label(
                "\(parsed.wires.count(where: { !$0.isClassical })) Qubits",
                systemImage: "circle.dotted.and.circle")
              Label(
                String(format: "Depth %.2f", currentStep.rawCriticalDepth),
                systemImage: "arrow.down.to.line.compact")
              Label(
                String(format: "Parallelism %.2f", currentStep.parallelism), systemImage: "bolt")
              Label(
                String(format: "Liveness %.2f", currentStep.liveness),
                systemImage: "waveform.path.ecg")
              Label(
                String(format: "Entanglement %.2f", currentStep.entanglementRatio),
      ...
      DashboardCardView(
        title: actionName, dotmarkColor: actionTypeEnum?.actionColor,
