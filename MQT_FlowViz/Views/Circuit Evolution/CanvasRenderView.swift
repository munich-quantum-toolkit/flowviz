...
struct CanvasRenderView: View {
  ...
  var body: some View {
    ...
    let (momentCenters, columnWidths) = calculateMomentCenters(
      circuit: currentCircuit, minWidth: defaultColumnWidth)

    // Total width is simply the last center + half its width + edge buffer
    let totalCalculatedWidth =
      (momentCenters.last ?? 0) + ((columnWidths.last ?? 0) / 2) + edgeBuffer
        ...
        Canvas { context, size in
          ...
          for op in currentCircuit.operations {
            ...
            case .barrier(let qubits):
              ...
              context.stroke(
                path, with: .color(.bluePrimary), style: StrokeStyle(lineWidth: 1.5, dash: [3, 3]))
  ...
  func calculateMomentCenters(circuit: ParsedCircuit, minWidth: CGFloat) -> (
    centers: [CGFloat], widths: [CGFloat]
  ) {
    ...
    for op in circuit.operations {
      ...
      case .singleQubit(_, let label, let parameter):
        let textLength =
          if let param = parameter { label.count + param.count + 2 } else { label.count }
