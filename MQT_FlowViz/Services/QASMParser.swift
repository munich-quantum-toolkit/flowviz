...
final class QASMParser {
  ...
  enum QASMParseError: LocalizedError {
    ...
    var errorDescription: String? {
      ...
      case .missingQubits(let line, let instruction):
        return
          "Syntax Error on line \(line): Expected target qubits for operation '\(instruction)'."
      ...
      case .emptyCircuit(let message):
        return
          "Parsing yielded empty circuit for QASM string: \"\(message)\" (\(message.count) characters)"
      }
    }
  }
  ...
  static func parse(qasm: String) throws -> ParsedCircuit {
    ...
    let lines = qasm.components(separatedBy: .newlines).map {
    ...
    for (index, line) in lines.enumerated() {
      ...
      if line.contains("measure") {
          ...
          let qBit = Int(qMatch), let cBit = Int(cMatch)
        {
          ...
          operations.append(
            CircuitOperation(
              type: .measurement(qubit: qBit, classicalBit: cBit), momentIndex: moment))
      ...
      if line.hasPrefix("barrier") {
        ...
        if !barrierQubits.isEmpty {
          ...
          operations.append(
            CircuitOperation(type: .barrier(qubits: barrierQubits), momentIndex: maxMoment))
      ...
      let gateIdentifierEndIndex =
        [qIndex, dollarIndex].compactMap { $0 }.min() ?? cleanLine.endIndex

      let rawGateIdentifier = String(cleanLine[..<gateIdentifierEndIndex]).trimmingCharacters(
        in: .whitespaces)
      ...
      if let invRange = baseGateIdentifier.range(
        of: "(?i)inv\\s*@\\s*", options: .regularExpression)
      {
        isInverse = true
      ...
      if let powRange = baseGateIdentifier.range(
        of: "(?i)pow\\(([^)]+)\\)\\s*@\\s*", options: .regularExpression)
      {
        // Extract the value inside the parentheses
        let matchString = String(baseGateIdentifier[powRange])
        if let paramStart = matchString.firstIndex(of: "("),
          let paramEnd = matchString.firstIndex(of: ")")
        {
      ...
      while let ctrlRange = baseGateIdentifier.range(
        of: "(?i)ctrl(?:\\(\\d+\\))?\\s*@\\s*", options: .regularExpression)
      {
      ...
      if let paramStart = baseGateIdentifier.firstIndex(of: "("),
        let paramEnd = baseGateIdentifier.lastIndex(of: ")")
      {
      ...
      if controls.isEmpty && targets.count == 1 {
        operations.append(
          CircuitOperation(
            type: .singleQubit(target: targets[0], label: gateName, parameter: parameter),
            momentIndex: finalMoment, classicalControls: currentlyActiveIfControlBits))
      } else if controls.count == 1 && targets.count == 1 {
        operations.append(
          CircuitOperation(
            type: .multiQubit(
              control: controls[0], target: targets[0], label: gateName, parameter: parameter),
            momentIndex: finalMoment, classicalControls: currentlyActiveIfControlBits))
      } else {
        operations.append(
          CircuitOperation(
            type: .nQubit(controls: controls, targets: targets, label: gateName),
            momentIndex: finalMoment, classicalControls: currentlyActiveIfControlBits))
      }
    ...
    let compactedOperations = operations.map { op -> CircuitOperation in
      ...
      case .singleQubit(let target, let label, let param):
        mappedType = .singleQubit(
          target: physicalToVisualMapQ[target]!, label: label, parameter: param)

      case .multiQubit(let control, let target, let label, let param):
        mappedType = .multiQubit(
          control: physicalToVisualMapQ[control]!, target: physicalToVisualMapQ[target]!,
  ...
  private static func formatParameter(_ gateLabel: String) -> String {
    do {
      ...
      let results = regex.matches(
        in: gateLabel, range: NSRange(location: 0, length: nsString.length))
      ...
      for result in results.reversed() {
        ...
        {
          formattedString = (formattedString as NSString).replacingCharacters(
            in: result.range, with: formattedValue)
        }
      }
